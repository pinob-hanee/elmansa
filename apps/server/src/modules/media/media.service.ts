...
export const generateSignedUrl = (key: string, resourceType: 'video' | 'image' | 'raw' | 'auto' = 'video', format?: string): string => {
  if (key.startsWith('http')) return key;
  if (!isCloudinaryConfigured) return `${env.APP_URL || 'http://localhost:5000'}/uploads/${key}`;
  
  const options: any = { secure: true, resource_type: resourceType };
